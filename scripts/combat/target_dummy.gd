...
@export var execute_ready_marks: int = 1
...
func get_overlay_text() -> String:
	...
	var reactor_hint := " | Reactor Ready" if is_reactor_ready() else ""
	return "HP %d/%d\nMark %d Seed %d Burn %d Freeze %d Greed %d | DPS %.1f%s%s" % [
...
func is_reactor_ready() -> bool:
	return not status.is_dead() and status.burn_stacks > 0 and status.freeze_stacks > 0
...
func _get_effect_color(tag: String) -> Color:
	match tag:
		...
		"summon_runner":
			return Color(0.38, 1.0, 0.7, 1.0)
		"summon_attach":
			return Color(1.0, 0.65, 0.32, 1.0)
		"summon_hit":
			return Color(0.72, 1.0, 0.84, 1.0)
		"burn":
...
func _get_effect_preset(tag: String) -> Dictionary:
	match tag:
		...
		"summon_runner":
			return {"color": Color(0.38, 1.0, 0.7, 0.95), "amount": 9, "lifetime": 0.3, "shape": "point", "spread": 40.0, "velocity_min": 0.2, "velocity_max": 0.7, "gravity_y": 2.8, "scale_min": 0.16, "scale_max": 0.28, "mesh_size": 0.25, "energy": 4.1}
		"summon_attach":
			return {"color": Color(1.0, 0.65, 0.32, 0.97), "amount": 10, "lifetime": 0.32, "shape": "sphere", "spread": 95.0, "velocity_min": 0.22, "velocity_max": 0.78, "gravity_y": 2.0, "scale_min": 0.18, "scale_max": 0.3, "mesh_size": 0.26, "energy": 4.5}
		"summon_hit":
			return {"color": Color(0.72, 1.0, 0.84, 0.96), "amount": 11, "lifetime": 0.28, "shape": "sphere", "spread": 130.0, "velocity_min": 0.25, "velocity_max": 0.92, "gravity_y": 2.2, "scale_min": 0.16, "scale_max": 0.28, "mesh_size": 0.24, "energy": 4.6}
		"burn":
		...
		"chain_lightning":
			return {"color": Color(1.0, 0.86, 0.22, 0.98), "amount": 12, "lifetime": 0.26, "shape": "point", "spread": 160.0, "velocity_min": 0.3, "velocity_max": 1.05, "gravity_y": 2.6, "scale_min": 0.16, "scale_max": 0.28, "mesh_size": 0.24, "energy": 4.9}
		_:
